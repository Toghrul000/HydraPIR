...
use crate::ms_kpir::bit_optimized_pir_service_private_update_server::BitOptimizedPirServicePrivateUpdateServer;
...
async fn run_server(addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    Server::builder()
        .add_service(BitOptimizedPirServicePrivateUpdateServer::new(pir_service))
        .serve(addr)
        .await?;
    Ok(())
}
