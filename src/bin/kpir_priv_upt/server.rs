use std::sync::Arc;
use dpf_half_tree_lib::{calculate_pir_config, dmpf_pir_query_eval_additive};
...
impl PirServicePrivateUpdate for MyPIRService {
    ...
    async fn private_update(
        ...
        let client_id = query.client_id;
        let server_id = query.server_id as usize;
        let update_key = query.update_key.ok_or_else(|| {
        ...
        let cw_n_proto = update_key.cw_n.ok_or_else(|| {
            Status::internal("Missing CW_n in DPFKey")
        })?;
        ...
        let seed: [u8; 16] = update_key.seed.try_into().map_err(|_| {
            Status::internal("Invalid seed size")
        })?;
        
        let cw_n = (hcw, cw_n_proto.lcw0 as u8, cw_n_proto.lcw1 as u8);
        
        let dpf_key = dpf_half_tree_lib::DPFKey_Bytes {
            n: update_key.n as usize,
            seed,
            cw_levels,
            cw_n,
            cw_np1: update_key.cw_np1.try_into().unwrap(),
        };
